...
class TwoDThreeD {
  ...
  // Real physical coordinates of the Lego board in mm
   ...
   float halfLength = boardSize/2.0f;
  ...
  public TwoDThreeD(int width, int height) {
   ...
   double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    for(int i=0;i<4;i++){
        ...
        // You can use Mat.multiply to multiply a matrix wit a vector.
        temp = points2D.get(i);
        
        projectedCorners[i] = Mat.multiply(invK, new float[]{temp.x, temp.y, 1});
    }
