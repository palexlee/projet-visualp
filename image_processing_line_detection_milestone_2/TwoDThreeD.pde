  ...
  public TwoDThreeD(int width, int height) {
   ...
   double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    for(int i=0;i<4;i++){
        ...
        PVector p = new PVector(points2D.get(i).x, points2D.get(i).y, 1);
        p.normalize();
        float[][] corner = { {points2D.get(i).x - img.width/2.0},
                            { img.height/2.0 - points2D.get(i).y},
                            {1}};
        println(corner[0][0] + " " + corner[1][0] + " " + corner[2][0]);
                        
        float[][] res = Mat.multiply(invK, corner);
        
        for(int j = 0; j < 3; ++j) {
          print(res[j][0] + " ");
          projectedCorners[i][j] = res[j][0]; 
        }
        println();
    }
